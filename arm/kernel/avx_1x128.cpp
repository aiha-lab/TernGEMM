...
void loop4_1x128(const int8_t* lhs, const int8_t* rhs, int32_t* dst,
    ...
    "add v6.16b, v6.16b, v2.16b\n"
    "add v7.16b, v7.16b, v3.16b\n"
...
void Scale_Shift_1x128(int32_t* dst, int8_t* result,
...
void dst_init_1x128(int32_t* dst) {
    asm volatile(
    "mov x0, %[dst]\n"
    "dup v0.4s, wzr\n"
...
void dst_add_1x128(int32_t* dst, int32_t* dst_temp) {
    asm volatile(
    "mov x0, %[dst_temp]\n"
...
void gemm_xor_avx_big(const int8_t* lhs, const int8_t* rhs, int8_t* dst,
    ...
    int64_t depth1 = (int64_t)(depth_last - 1) / 16;
    int64_t depth2 = (int64_t)(depth_last - 1) % 16 + 1;
    int64_t depth3 = (int64_t)(depth_last_2nd - 1) / 16;
    int64_t depth4 = (int64_t)(depth_last_2nd - 1) % 16 + 1;

    auto rhs_m = new int8_t[depth * 128];
    auto dst_temp = new int32_t[128];
    auto dst_acc = new int32_t[height * 128];

    for (int i = 0; i < (width - 1) / 128; i++) {
        for (int b = 0; b < height; b++) {
            dst_init_1x128(dst_acc + 128 * b);
        }
        for (int k = 0; k < depth_outer; k++) {
            for (int a = 0; a < 256; a++) {
                memcpy(rhs_m + a * 128, rhs + ((a + k * 256) * width + 128 * i), 128 * sizeof(int8_t));
            }
            for (int j = 0; j < height; j++) {
                loop4_1x128(lhs + depth * j + k * 256,
                            rhs_m,
                            dst_temp, 15, 16);
                dst_add_1x128(dst_acc + 128 * j, dst_temp);
            }
        }
        for (int a = 0; a < depth_last_2nd; a++) {
            memcpy(rhs_m + a * 128, rhs + ((a + depth_outer * 256) * width + 128 * i), 128 * sizeof(int8_t));
        }
        for (int j = 0; j < height; j++) {
            loop4_1x128(lhs + depth * j + depth_outer * 256,
                        rhs_m,
                        dst_temp, depth3, depth4);
            dst_add_1x128(dst_acc + 128 * j, dst_temp);
        }
        for (int a = 0; a < depth_last; a++) {
            memcpy(rhs_m + a * 128, rhs + ((a + depth_outer * 256 + depth_last_2nd) * width + 128 * i), 128 * sizeof(int8_t));
        }
        for (int j = 0; j < height; j++) {
            loop4_1x128(lhs + depth * j + depth_outer * 256 + depth_last_2nd,
                        rhs_m,
                        dst_temp, depth1, depth2);
            dst_add_1x128(dst_acc + 128 * j, dst_temp);
            Scale_Shift_1x128(dst_acc + 128 * j, dst + width * j + 128 * i,
    ...
    for (int k = 0; k < depth_outer; k++) {
        for (int a = 0; a < 256; a++) {
            memcpy(rhs_m + a * 128, rhs + ((a + k * 256) * width + width - 128), 128 * sizeof(int8_t));
        }
        for (int j = 0; j < height; j++) {
            loop4_1x128(lhs + depth * j + k * 256,
                        rhs_m,
    ...
    for (int a = 0; a < depth_last_2nd; a++) {
        memcpy(rhs_m + a * 128, rhs + ((a + depth_outer * 256) * width + width - 128), 128 * sizeof(int8_t));
    }
    for (int j = 0; j < height; j++) {
        loop4_1x128(lhs + depth * j + depth_outer * 256,
                    rhs_m,
    ...
    for (int a = 0; a < depth_last; a++) {
        memcpy(rhs_m + a * 128, rhs + ((a + depth_outer * 256 + depth_last_2nd) * width + width - 128), 128 * sizeof(int8_t));
    }
    for (int j = 0; j < height; j++) {
        loop4_1x128(lhs + depth * j + depth_outer * 256 + depth_last_2nd,
                    rhs_m,
        ...
        if (width >= 128)
            Scale_Shift_1x128(dst_acc + 128 * j, dst + width * j + width - 128,
                              rhs_offset, dst_offset, scale, shift);
        else {
            Scale_Shift_1x128(dst_acc + 128 * j, dst + width * j,
                              rhs_offset, dst_offset, scale, shift);
        }
    }
...
void gemm_xor_avx_small(const int8_t* lhs, const int8_t* rhs, int8_t* dst,
    ...
    int8_t* rhs_m = new int8_t[depth * 128];
    ...
    for (int i = 0; i < (width - 1) / 128; i++) {
        ...
        for (int j = 0; j < height; j++) {
            loop4_1x128(lhs + depth * j,
                        rhs_m,
                        dst_temp, depth1, depth2);

            Scale_Shift_1x128(dst_temp, dst + width * j + 128 * i,
                              rhs_offset, dst_offset, scale, shift);
        }
    }
    ...
    for (int j = 0; j < height; j++) {
        ...
        if (width >= 128) {
            Scale_Shift_1x128(dst_temp, dst + width * j + width - 128,
                              rhs_offset, dst_offset, scale, shift);
        }
        else {
            Scale_Shift_1x128(dst_temp, dst + width * j,
                              rhs_offset, dst_offset, scale, shift);
        }
    }
