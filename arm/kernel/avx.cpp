...
void gemm_xor_avx_big(const int8_t* lhs, const int8_t* rhs, int8_t* dst,
    ...
    for (int i = 0; i < (width - 1) / 96; i++) {
        for (int b = 0; b < height; b++) {
            dst_init(dst_acc + 96 * b);
        }
        for (int k = 0; k < depth_outer; k++) {
            for (int a = 0; a < 256; a++) {
                memcpy(rhs_m + a * 96, rhs + ((a + k * 256) * width + 96 * i), 96 * sizeof(int8_t));
            }
            for (int j = 0; j < height; j++) {
                loop4(lhs + depth * j + k * 256,
                      rhs_m,
                      dst_temp, 15, 16);
                dst_add(dst_acc + 96 * j, dst_temp);
            }
        }
        for (int a = 0; a < depth_last_2nd; a++) {
            memcpy(rhs_m + a * 96, rhs + ((a + depth_outer * 256) * width + 96 * i), 96 * sizeof(int8_t));
        }
        for (int j = 0; j < height; j++) {
            loop4(lhs + depth * j + depth_outer * 256,
                  rhs_m,
                  dst_temp, depth3, depth4);
            dst_add(dst_acc + 96 * j, dst_temp);
        }
        for (int a = 0; a < depth_last; a++) {
            memcpy(rhs_m + a * 96, rhs + ((a + depth_outer * 256 + depth_last_2nd) * width + 96 * i), 96 * sizeof(int8_t));
        }
        for (int j = 0; j < height; j++) {
            loop4(lhs + depth * j + depth_outer * 256 + depth_last_2nd,
                  rhs_m,
                  dst_temp, depth1, depth2);
            dst_add(dst_acc + 96 * j, dst_temp);
            Scale_Shift(dst_acc + 96 * j, dst + width * j + 96 * i,
    ...
    for (int k = 0; k < depth_outer; k++) {
        for (int a = 0; a < 256; a++) {
            memcpy(rhs_m + a * 96, rhs + ((a + k * 256) * width + width - 96), 96 * sizeof(int8_t));
        }
        for (int j = 0; j < height; j++) {
            loop4(lhs + depth * j + k * 256,
                  rhs_m,
    ...
    for (int a = 0; a < depth_last_2nd; a++) {
        memcpy(rhs_m + a * 96, rhs + ((a + depth_outer * 256) * width + width - 96), 96 * sizeof(int8_t));
    }
    for (int j = 0; j < height; j++) {
        loop4(lhs + depth * j + depth_outer * 256,
              rhs_m,
    ...
    for (int a = 0; a < depth_last; a++) {
        memcpy(rhs_m + a * 96, rhs + ((a + depth_outer * 256 + depth_last_2nd) * width + width - 96), 96 * sizeof(int8_t));
    }
    for (int j = 0; j < height; j++) {
        loop4(lhs + depth * j + depth_outer * 256 + depth_last_2nd,
              rhs_m,
        ...
        if (width >= 96)
            Scale_Shift(dst_acc + 96 * j, dst + width * j + width - 96,
                        rhs_offset, dst_offset, scale, shift);
        else {
            Scale_Shift(dst_acc + 96 * j, dst + width * j,
                        rhs_offset, dst_offset, scale, shift);
        }
    }
