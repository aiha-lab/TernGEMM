...
int main(int argc, char** argv) {
    ...
    // Load binary
    // ---------------------------------------------------------------- //
    auto lhs_m = (int8_t*) _mm_malloc(height * depth * sizeof(int8_t) + 256, 256);
    auto rhs_m = (int8_t*) _mm_malloc(width * depth * sizeof(int8_t) + 256, 256);
    ...
    // Run
    // ---------------------------------------------------------------- //
    auto tmp8 = (int8_t*) _mm_malloc(256 * 128 * sizeof(int8_t) + 256, 256);
    auto tmp32b = (int32_t*) _mm_malloc(height * 128 * sizeof(int32_t) + 256, 256);
//    if (depth < 256) {
//        _mm_free(tmp8);
//        tmp8 = (int8_t*) _mm_malloc(depth * 128 * sizeof(int8_t), 256);
//    }
    auto tmp32a = (int32_t*) _mm_malloc(128 * sizeof(int32_t) + 256, 256);
    ...
    // run avx warmup
    if (bit == 3) {
//        gemm_xor_avx_b3_1x96_new(lhs_m, rhs_m, dst_m_avx,
//                             tmp8, tmp32a, tmp32b,
//                             height, depth, width,
//                             rhs_offset, dst_offset, scale, shift);
    }
    else if (bit == 4) {
        gemm_xor_avx_b4_1x96_new(lhs_m, rhs_m, dst_m_avx,
                                 tmp8, tmp32a, tmp32b,
                                 height, depth, width,
                                 rhs_offset, dst_offset, scale, shift);
    }
...
//        gemm_xor_avx_b6_1x96_new(lhs_m, rhs_m, dst_m_avx,
//                             tmp8, tmp32a, tmp32b,
//                             height, depth, width,
//                             rhs_offset, dst_offset, scale, shift);
    }
...
//    int64_t NUM_RUN = 171798000 / (height * width * depth);  // 2^34 / (...)
...
//            gemm_xor_avx_b5_1x96_new(lhs_m, rhs_m, dst_m_avx,
//                                 tmp8, tmp32a, tmp32b,
//                                 height, depth, width,
//                                 rhs_offset, dst_offset, scale, shift);
        }
