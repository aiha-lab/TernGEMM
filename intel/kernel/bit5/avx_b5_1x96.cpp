...
inline void loop_b5_1x96(const int8_t* lhs, const int8_t* rhs, int32_t* dst,
...
void gemm_xor_avx_small_b5_1x96(const int8_t* lhs, const int8_t* rhs, int8_t* dst,
                                int height, int depth, int width,
                                int32_t* rhs_offset, int32_t* dst_offset,
                                int32_t* scale, int32_t* shift) {

    int64_t depth1 = (int64_t) (depth - 1) / 7;
    int64_t depth2 = (int64_t) (depth - 1) % 7 + 1;
...
void gemm_xor_avx_big_b5_1x96(const int8_t* lhs, const int8_t* rhs, int8_t* dst,
                              int height, int depth, int width,
                              int32_t* rhs_offset, int32_t* dst_offset,
                              int32_t* scale, int32_t* shift) {

    int32_t depth_outer = (depth - 1) / 2047 - 1;
    int32_t depth_last_2nd = 2047;
    int32_t depth_last = (depth - 1) % 2047 + 1;
    if (depth_last < 96) {
        depth_last_2nd -= 95;
        depth_last += 95;
    }

    int64_t depth1 = (int64_t) (depth_last - 1) / 7;
    int64_t depth2 = (int64_t) (depth_last - 1) % 7 + 1;
    int64_t depth3 = (int64_t) (depth_last_2nd - 1) / 7;
    int64_t depth4 = (int64_t) (depth_last_2nd - 1) % 7 + 1;
    ...
    for (int i = 0; i < (width - 1) / 96; i++) {
        ...
        for (int j = 0; j < height; j++) {
            ...
            for (int k = 0; k < depth_outer; k++) {
                loop_b5_1x96(lhs + depth * j + k * 2047,
                             rhs_m + k * 2047 * 96,
                             dst_temp, 292, 3);
                dst_add_1x96(dst_acc, dst_temp);
            }

            loop_b5_1x96(lhs + depth * j + depth_outer * 2047,
                         rhs_m + depth_outer * 2047 * 96,
                         dst_temp, depth3, depth4);
            dst_add_1x96(dst_acc, dst_temp);

            loop_b5_1x96(lhs + depth * j + depth_outer * 2047 + depth_last_2nd,
                         rhs_m + (depth_outer * 2047 + depth_last_2nd) * 96,
                         dst_temp, depth1, depth2);
            dst_add_1x96(dst_acc, dst_temp);
...
void gemm_xor_avx_b5_1x96(const int8_t* lhs, const int8_t* rhs, int8_t* dst,
                          ...
                          int32_t* scale, int32_t* shift) {
    if (depth < 2048) {
        gemm_xor_avx_small_b5_1x96(lhs, rhs, dst, height, depth, width, rhs_offset, dst_offset, scale, shift);
    }
    else {
        gemm_xor_avx_big_b5_1x96(lhs, rhs, dst, height, depth, width, rhs_offset, dst_offset, scale, shift);
    }
}
