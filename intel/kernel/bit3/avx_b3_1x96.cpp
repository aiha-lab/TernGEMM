...
inline void loop_b3_1x96(const int8_t* lhs, const int8_t* rhs, int32_t* dst,
...
void gemm_xor_avx_small_b3_1x96(const int8_t* lhs, const int8_t* rhs, int8_t* dst,
                                int height, int depth, int width,
                                int32_t* rhs_offset, int32_t* dst_offset,
                                int32_t* scale, int32_t* shift) {

    int64_t depth1 = (int64_t) (depth - 1) / 31;
    int64_t depth2 = (int64_t) (depth - 1) % 31 + 1;
...
void gemm_xor_avx_big_b3_1x96(const int8_t* lhs, const int8_t* rhs, int8_t* dst,
                              int height, int depth, int width,
                              int32_t* rhs_offset, int32_t* dst_offset,
                              int32_t* scale, int32_t* shift) {
    int32_t depth_outer = (depth - 1) / 8191 - 1;
    int32_t depth_last_2nd = 8191;
    int32_t depth_last = (depth - 1) % 8191 + 1;

    if (depth_last < 96) {
        depth_last_2nd -= 95;
        depth_last += 95;
    }
    int64_t depth1 = (int64_t) (depth_last - 1) / 31;
    int64_t depth2 = (int64_t) (depth_last - 1) % 31 + 1;
    int64_t depth3 = (int64_t) (depth_last_2nd - 1) / 31;
    int64_t depth4 = (int64_t) (depth_last_2nd - 1) % 31 + 1;

    auto rhs_m = (int8_t*) _mm_malloc(depth * 96 * sizeof(int8_t), 256);
    auto dst_temp = (int32_t*) _mm_malloc(96 * sizeof(int32_t), 256);
    auto dst_acc = (int32_t*) _mm_malloc(96 * sizeof(int32_t), 256);
    ...
    for (int i = 0; i < (width - 1) / 96; i++) {
        ...
        for (int j = 0; j < height; j++) {
            dst_init_1x96(dst_acc);
            for (int k = 0; k < depth_outer; k++) {
                loop_b3_1x96(lhs + depth * j + k * 8191,
                             rhs_m + k * 8191 * 96,
                             dst_temp, 264, 11);
                dst_add_1x96(dst_acc, dst_temp);
            }

            loop_b3_1x96(lhs + depth * j + depth_outer * 8191,
                         rhs_m + depth_outer * 8191 * 96,
                         dst_temp, depth3, depth4);
            dst_add_1x96(dst_acc, dst_temp);

            loop_b3_1x96(lhs + depth * j + depth_outer * 8191 + depth_last_2nd,
                         rhs_m + (depth_outer * 8191 + depth_last_2nd) * 96,
                         dst_temp, depth1, depth2);
            dst_add_1x96(dst_acc, dst_temp);

            scale_shift_x12(dst_acc, dst + width * j + 96 * i,
                            rhs_offset, dst_offset, scale, shift);
        }
    }
    ...
    for (int j = 0; j < height; j++) {
        dst_init_1x96(dst_acc);
        for (int k = 0; k < depth_outer; k++) {
            loop_b3_1x96(lhs + depth * j + k * 8191, rhs_m + k * 8191 * 96, dst_temp, 264, 11);
            dst_add_1x96(dst_acc, dst_temp);
        }
        ...
        if (width >= 96) {
            scale_shift_x12(dst_acc, dst + width * j + width - 96,
                            rhs_offset, dst_offset, scale, shift);
        }
        else {
            scale_shift_x12(dst_acc, dst + width * j,
                            rhs_offset, dst_offset, scale, shift);
        }
    }
...
void gemm_xor_avx_b3_1x96(const int8_t* lhs, const int8_t* rhs, int8_t* dst,
                          ...
                          int32_t* scale, int32_t* shift) {
    if (depth < 8192) {
        gemm_xor_avx_small_b3_1x96(lhs, rhs, dst, height, depth, width, rhs_offset, dst_offset, scale, shift);
    }
    else {
        gemm_xor_avx_big_b3_1x96(lhs, rhs, dst, height, depth, width, rhs_offset, dst_offset, scale, shift);
    }
}
