...
int main(int argc, char** argv) {
    ...
    ifstream is;
    is.open("/home/shchoi/SIPS2021/src/parameters/resnet_param_0.bin", ios::binary);
    auto buffer = new char [64];
    is.read(buffer, 64);
    is.close();
    auto n_arr = (int32_t*) _mm_malloc(64*sizeof(int32_t), 256);
    for(int i=0; i<64; i++){
        n_arr[i]=static_cast<int32_t>(buffer[i]);
    }
    ...
    if (bit == 3) {
        ...
        //gemm_xor_avx_b4_1x96_n(lhs_m, rhs_m, dst_m_avx, height, depth, width,
        //                     rhs_offset, dst_offset, scale, shift, n_arr);
        gemm_xor_avx_b4_1x96(lhs_m, rhs_m, dst_m_avx, height, depth, width,
                             rhs_offset, dst_offset, scale, shift);

    } else if (bit == 5) {
        gemm_xor_avx_b5_1x96(lhs_m, rhs_m, dst_m_avx, height, depth, width,
                             rhs_offset, dst_offset, scale, shift);
    } else if (bit == 6) {
        gemm_xor_avx_b6_1x96(lhs_m, rhs_m, dst_m_avx, height, depth, width,
                             rhs_offset, dst_offset, scale, shift);
    }
    }
    ...
    for (int count = 0; count < NUM_RUN; count++) {
        ...
        if (bit == 3) {
            gemm_xor_avx_b3_1x96(lhs_m, rhs_m, dst_m_avx, height, depth, width,
                                 rhs_offset, dst_offset, scale, shift);
        }
        else if (bit == 4) {
            //gemm_xor_avx_b4_1x96_n(lhs_m, rhs_m, dst_m_avx, height, depth, width,
